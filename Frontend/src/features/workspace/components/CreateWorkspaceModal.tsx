        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-xs px-4">
            <div className="w-full max-w-md bg-white rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-zinc-200 overflow-visible">
                <div className="px-6 py-4 border-b border-zinc-100 flex justify-between items-center bg-zinc-50/50 rounded-t-xl">
                    <h3 className="font-semibold text-base text-zinc-900">Create workspace</h3>
                    <button type="button" onClick={() => setWorkspaceModal(false)} className="text-zinc-400 hover:text-zinc-700 transition-colors cursor-pointer p-1 rounded-md hover:bg-zinc-100">
                        <X size={18} />
                    </button>
                </div>
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea
                            className={cn(
                                "block w-full rounded-lg border border-zinc-200 bg-white px-3.5 py-2.5 text-sm text-zinc-900 transition-all resize-none",
                                "placeholder:text-zinc-400",
                                "focus:border-black focus:outline-none focus:ring-1 focus:ring-black"
                            )}
                            value={description}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-zinc-700 mb-1.5">Members</label>
                        <div className="relative">
                            <div
                                onClick={() => setSelectBoxOpen(!selectBoxOpen)}
                                className={cn(
                                    "flex items-center justify-between w-full rounded-lg border bg-white px-3.5 py-2.5 text-sm text-zinc-900 transition-all cursor-pointer",
                            ...
                            >
                                ...
                                <ChevronRight size={15} className={`text-zinc-400 transition-transform duration-150 ${selectBoxOpen ? 'rotate-90' : ''}`} />
                            </div>
                                    ...
                                    {users.map(u => (
                                        ...
                                        >
                                            <input 
                                                ...
                                                type="checkbox" 
                                                id={u._id} 
                                                className="w-4 h-4 rounded border-zinc-300 text-black focus:ring-0 cursor-pointer accent-black"
                                            />
